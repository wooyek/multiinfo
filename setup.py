...
version = get_version("src", "multiinfo", "__init__.py")
...
setup(
    name='multiinfo',
    version=version,
    description="""Python wrapper for Polkomtel's MultiInfo SMS service""",
    long_description=readme + '\n\n' + history,
    author="Janusz Skonieczny",
    author_email='js+pypi@bravelabs.pl',
    url='https://github.com/wooyek/multiinfo',
    packages=find_packages('src', exclude=["*.tests", "*.tests.*", "tests.*", "tests"]),
    package_dir={'': 'src'},
    py_modules=[splitext(basename(path))[0] for path in glob('src/*.py')],
    entry_points={
        'console_scripts': [
            'multiinfo=multiinfo.cli:main'
        ]
    },
