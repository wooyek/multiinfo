...
def check(ctx):
    ...
    ctx.run("pytest --cov=src --cov=tests --cov-fail-under=60")
...
def bump(ctx):
    ...
    ctx.run("bumpversion minor --no-tag")
...
def pipenv(ctx):
    ...
    ctx.run('pipenv lock --requirements --dev | grep -v "/multiinfo" -- > requirements/lock/development.txt')
